...
def create_plot(
    net_payoff_list, stock_price_range, title, max_gain=0, max_loss=0, breakeven=[0]
):
    fig, ax = plt.subplots(figsize=(10, 8))
    ax.spines["bottom"].set_position("zero")

    for i in range(len(net_payoff_list)):
        if (len(net_payoff_list) > 1 and i == len(net_payoff_list) - 1) or len(
            net_payoff_list
        ) == 1:
            ax.plot(
                stock_price_range, net_payoff_list[i][0], label=net_payoff_list[i][1]
            )
        else:
            ax.plot(
                stock_price_range,
                net_payoff_list[i][0],
                "--",
                label=net_payoff_list[i][1],
            )

    format_max_gain = (
        "Max Gain: $" + str("{:6.2f}").format(max_gain)
        if type(max_gain) != str
        else "Max Gain: $inf"
    )
    format_max_loss = (
        "Max Loss: $" + str("{:6.2f}").format(max_loss)
        if type(max_loss) != str
        else "Max Loss: $inf"
    )
    ...
    for i in range(len(breakeven)):
        ax.text(
            0.75,
            (-0.10 / float(i + 1)),
            "Breakeven Price: $" + str("{:6.2f}").format(breakeven[i]),
            horizontalalignment="center",
            verticalalignment="center",
            color="black",
            transform=ax.transAxes,
...
def spread_gain_loss(
    strike_price_long, premium_long, strike_price_short, premium_short, credit=True
):
    max_gain = abs((premium_short - premium_long) * 100)
    max_loss = (
        abs(strike_price_long - strike_price_short) - abs(premium_short - premium_long)
    ) * 100
...
def call_option(
    underlying_price, strike_price, premium, stock_price_range, short=False, plot=False
):
    profit_loss = [
        value - strike_price - premium if value > strike_price else 0 - premium
        for value in stock_price_range
    ]
    profit_loss = np.negative(profit_loss) if short else profit_loss

    if plot:
        line_name, max_gain, max_loss, breakeven = call_or_put_gain_loss_breakeven(
            strike_price, premium, True, short
        )
        create_plot(
...
def put_option(
    ...
    profit_loss = [
        strike_price - value - premium if strike_price > value else 0 - premium
        for value in stock_price_range
    ]
    profit_loss = np.negative(profit_loss) if short else profit_loss
    ...
    if plot:
        line_name, max_gain, max_loss, breakeven = call_or_put_gain_loss_breakeven(
            strike_price, premium, False, short
        )
        create_plot(
            [(profit_loss, line_name)],
            stock_price_range,
...
def call_credit_or_debit_spread(
    underlying_price,
    strike_price_long,
    premium_long,
    strike_price_short,
    ...
    max_gain, max_loss = spread_gain_loss(
        strike_price_long, premium_long, strike_price_short, premium_short, credit
    )
    breakeven = (
        strike_price_short + premium_short - premium_long
        if credit
        else strike_price_long + premium_long - premium_short
    )
...
def put_credit_or_debit_spread(
    underlying_price,
    strike_price_long,
    premium_long,
    strike_price_short,
    ...
    breakeven = (
        strike_price_short + premium_long - premium_short
        if credit
        else strike_price_long + premium_short - premium_long
    )
...
def strangle_or_straddle(
    ...
    net_payoff_list = []
    str_short_or_long_title = (
        "Short Strangle or Straddle" if short else "Long Strangle or Straddle"
    )
    ...
    long_put_payoff = put_option(
        underlying_price, strike_price_put, premium_put, stock_price_range, short, True
    )
...
def spreads(
    ...
    if right == "call":
        title = (
            "Bear Call Spread"
            if strike_price_short < strike_price_long
        ...
        raise ValueError(
            f"`right` must be call/put. Values entered are right: {right}."
        )
    ...
    if right == "call":
        ...
        net_payoff_list.append((np.add(long_call_payoff, short_call_payoff), title))
        ...
        net_payoff_list.append((np.add(long_put_payoff, short_put_payoff), title))

    # net_debit = premium_long - premium_short
    # net_credit = premium_short - premium_long

    # max_spread = abs(strike_price_long - strike_price_short)
    ...
    # breakeven_upper = (
    #     strike_price_long_call + net_debit
    #     if not short
    #     else strike_price_short_call + net_credit
    # )
    # breakeven_lower = (
    #     strike_price_long_put - net_debit
    #     if not short
    #     else strike_price_short_put - net_credit
    # )
...
def iron_condor(
    ...
    premium_long_put,
    strike_price_short_put,
    premium_short_put,
    strike_price_short_call,
    premium_short_call,
    ...
    net_debit = (premium_long_put + premium_long_call) - (
        premium_short_put + premium_short_call
    )
    net_credit = (premium_short_put + premium_short_call) - (
    ...
    max_spread = (
        spread_width_puts
        if spread_width_puts >= spread_width_calls
        else spread_width_calls
    )
